...
class HttpClientUS extends HttpClient
{
    ...
    private function send(
      ...
      ?array $payload = []
        ...
        $response = $this->exceptionHandler->transformResponseToException(
          $request,
          $response
        );
    ...
    private function createRequest(
      ...
      ?array $payload = []
        ...
        if (null !== $this->api_key && null !== $this->api_secret) {
            ...
            $request = $request->withHeader(
              'FTXUS-SIGN',
              $this->calculateSignature($timestamp, $request, $payload)
            );
        }
    ...
    private function calculateSignature(
      ...
      array $payload = []
        ...
        $data = $timestamp.$request->getMethod().$request->getUri()->getPath(
            ).($request->getUri()->getQuery() ? '?'.$request->getUri()
              ->getQuery() : '').(($request->getMethod(
            ) == 'POST' || $request->getMethod() == 'DELETE') ? json_encode(
            $payload
          ) : '');
